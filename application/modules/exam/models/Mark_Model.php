...
class Mark_Model extends MY_Model {
    ...
    public function get_student_list_by_class($exam_id, $class_id, $receiver_id){
    ...
    public function get_receiver($role_id, $student_id, $guardian_id){
        
        if($role_id == STUDENT){
            
            $this->db->select('U.id, U.email, U.role_id,  S.name, S.phone');
